...
@section('title', $currentAuthor->name . ' - ' . $currentSite->title)
@section('meta_description', 'Artigos escritos por ' . $currentAuthor->name . ' em ' . $currentSite->title)

@section('content')
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    
    <!-- Perfil do Autor -->
    <div class="mb-12 flex flex-col md:flex-row items-center gap-6 bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
        @if($currentAuthor->avatar_url)
            <img src="{{ $currentAuthor->avatar_url }}" alt="{{ $currentAuthor->name }}" class="w-32 h-32 rounded-full object-cover shadow-sm">
        @else
            <div class="w-32 h-32 rounded-full bg-primary/10 flex items-center justify-center text-primary font-bold text-4xl shadow-sm">
                {{ substr($currentAuthor->name, 0, 1) }}
            </div>
        @endif
    ...
    <div class="flex items-center justify-between mb-8">
        <h2 class="text-2xl font-bold text-gray-900">Publicações Recentes</h2>
    </div>
                ...
                <article class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow flex flex-col h-full">
                    ...
                    <div class="p-6 flex flex-col flex-grow">
                        ...
                        <div class="flex items-center gap-3 mb-3 text-sm">
                            @if($article->category)
                                <a href="/categoria/{{ $article->category->slug }}" class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-primary/10 text-primary hover:bg-primary/20 transition-colors">
    ...
    @else
        ...
        <div class="text-center py-24 bg-white rounded-2xl shadow-sm border border-gray-100">
            ...
            <h3 class="mt-4 text-sm font-medium text-gray-900">Nenhum artigo publicado ainda</h3>
            <p class="mt-1 text-sm text-gray-500">Este autor ainda não publicou nenhum artigo.</p>
        </div>
    @endif
</div>
@endsection
