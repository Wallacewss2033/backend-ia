...
@section('title', $currentSite->title . ' - Início')
@section('meta_description', $currentSite->description ?? 'Confira os últimos artigos e novidades.')

@section('content')
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    
    <!-- Seção Institucional / Hero -->
    <div class="mb-12 text-center max-w-3xl mx-auto">
        <h1 class="text-4xl font-extrabold text-gray-900 tracking-tight sm:text-5xl mb-4">
            Bem-vindo(a) ao <span class="text-primary">{{ $currentSite->title }}</span>
        </h1>
        @if($currentSite->description)
            <p class="text-xl text-gray-500">{{ $currentSite->description }}</p>
        @endif
    </div>

    <!-- Grid de Artigos Recentes -->
    <div class="flex items-center justify-between mb-8">
        <h2 class="text-2xl font-bold text-gray-900">Artigos Recentes</h2>
    </div>
    ...
    @else
        ...
        <div class="text-center py-24 bg-white rounded-2xl shadow-sm border border-gray-100">
            ...
            <p class="mt-1 text-sm text-gray-500">Volte mais tarde para conferir as novidades.</p>
        </div>
    @endif
</div>
@endsection
